...
//
// API
// featurePyramid* createFeaturePyramidWithBorder(const IplImage *image, int maxXBorder, int maxYBorder);
...
{
    (*fft_obj) = ( CvLSVMFftPyramid * )malloc( sizeof(CvLSVMFftPyramid) );
    (*fft_obj)->numLevels = numLevels;
    (*fft_obj)->pyramid = ( CvLSVMFftImage ** )malloc( sizeof(CvLSVMFftImage *) * numLevels );
...
int freeFftFeaturePyramidObject( CvLSVMFftPyramid ** fft_obj )
{
    ...
    for( int i = 0; i < (* fft_obj)->numLevels; i ++ )
    {
