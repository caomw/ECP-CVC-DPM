...
JPEGImage::JPEGImage( const cv::Mat img )
{
	Mat_<Vec3b> _img = img;
	width_  = img.cols;
	height_ = img.rows;
	...
	//vector<uint8_t> bits( width_ * height_ * depth_ );
	int num = 0;
	for( int i = 0; i < height_; ++ i )
	{
		for( int j = 0; j < width_; ++ j )
		{
			...
			bits_.push_back( uint8_t( _img(i,j)[0] ) );
			bits_.push_back( uint8_t( _img(i,j)[1] ) );
			bits_.push_back( uint8_t( _img(i,j)[2] ) );
			//bits_[ (i * width_ + j) * depth_ + 1 ] = uint8_t( _img(i,j)[1] );
			//bits_[ (i * width_ + j) * depth_ + 2 ] = uint8_t( _img(i,j)[2] );
...
JPEGImage::JPEGImage(const string & filename) : width_(0), height_(0), depth_(0)
{
	...
	if ((jpeg_read_header(&cinfo, TRUE) != JPEG_HEADER_OK) || (cinfo.data_precision != 8) || !jpeg_start_decompress(&cinfo)) 
	{
		fclose(file);
		return;
	}
...
JPEGImage JPEGImage::resize(int width, int height) const
{
	...
	Resize(tmpSrc.empty() ? &bits_[0] : &tmpSrc[0], srcWidth, srcHeight, &result.bits_[0], width, height, depth_);
...
{
	// Empty image
	if ((width <= 0) || (height <= 0) || (x + width <= 0) || (y + height <= 0) || (x >= width_) || (y >= height_))
	{
		return JPEGImage();
	}
