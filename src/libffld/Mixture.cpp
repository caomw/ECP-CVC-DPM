...
void Mixture::importFromOpenCVModel( int numFilters, int numComponents, int* numPartFilters, float* bias, CvLSVMFilterObject** filters )
{
	//int nbModels = numFilters;

	// for( int i = 0; i < nbModels; i ++ )
	// {
	// 	Model model;
	// 	cout<<i<<":"<<filters[i][0].numFeatures<<endl;
	// 	// model.importFromOpenCVModel( numComponents, numPartFilters, bias,  );
	// 	//models_.push_back( model );
	// }

	// for( int i = 0; i < numComponents; i ++ )
	// {
	// 	cout<<"Part:"<<numPartFilters[i]<<endl;
	// }
	...
	for( int i = 0; i < numComponents; i ++ )
	{
		...
		{
			nStart = 0;
			nEnd = numPartFilters[i];
		}
		else
		{
			nStart = nEnd + 1;
			nEnd += numPartFilters[i] + 1;
		}

		model.importOpenCVModel( nStart, nEnd, i, bias, filters );
		models_.push_back( model );
	}
