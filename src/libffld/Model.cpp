...
void Model::importOpenCVModel( int start, int end, int nModelID, float* bias, CvLSVMFilterObject** filters )
{
	bias_ = bias[nModelID];

	parts_.resize( end - start + 1 );
	for( int i = start; i <= end; i ++ )
	{
		int k = i - start;
		int cols, rows, nFeatures;
		CvLSVMFilterObject pPartFilter = filters[i][0];

		cols = pPartFilter.sizeX;
		rows = pPartFilter.sizeY;
		nFeatures = pPartFilter.numFeatures;

		//cout<<cols<<" "<<rows<<" "<<nFeatures<<endl;

		//cout<<"x"<<parts_[k].offset(0)<<endl;
		//cout<<"y"<<parts_[k].offset(1)<<endl;

		if( k == 0 )
		{
			parts_[k].deformation.setZero();
			parts_[k].offset.setZero();
		}
		else
		{
			parts_[k].offset(0) = pPartFilter.V.x - 1;
			parts_[k].offset(1) = pPartFilter.V.y - 1;
		
			
			parts_[k].deformation(0) = -pPartFilter.fineFunction[2];
			parts_[k].deformation(1) = -pPartFilter.fineFunction[0];
			parts_[k].deformation(2) = -pPartFilter.fineFunction[3];
			parts_[k].deformation(3) = -pPartFilter.fineFunction[1];
		}

		parts_[k].filter.resize(rows, cols);
		for (int y = 0; y < rows; y ++ ) 
		{
			for (int x = 0; x < cols; x ++ ) 
			{
				...
				{
					parts_[k].filter(y, x)(j) =	pPartFilter.H[ y * (cols) * nFeatures + x * nFeatures + j ];
				}
			}
		}
